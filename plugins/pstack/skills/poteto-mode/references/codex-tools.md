...
`skills/poteto-mode/scripts/` ships the `watch-pr` PR watcher, the `orch` store CLI, and `worktree_audit.py`. The `watch-pr/ship-pr` command owns pending-merge inspection and cancellation; `resume.mjs` owns the shared checkpoint locator described in [Resume storage](resume-storage.md). `watch-pr`, `orch`, and `resume.mjs` run on bun and Node.js; `worktree_audit.py` runs on Python 3.12 and its standard library alone. All of them run the same on Codex; invoke them through `shell`. They need `bun`, `gh`, (for stack work) `gt`, and (for `worktree_audit.py`) `python3`, `git`, and `du`. `worktree_audit.py` reads Claude Code transcripts under `~/.claude/projects/`; point it at your runtime's transcript directory instead when you run it elsewhere.
