...
- Before any PR opened or integrated -> `skeptic-gate`.
...
Every spawn carry the brief fields in `references/brief.md`. Field you cannot fill = task not scoped. Model pinned per call from `models.md`, never frontmatter. Constraints live in FORBIDDEN, not tool config. Paste the user's global instructions into every spawn; directives decay.
...
Large or cross-cutting effort, or no bundled playbook fits -> `figure-it-out`. Standing multi-day program, many units, fleet of subagents under one coordinator -> Orchestrate.
...
- **Orchestrate.** Program outliving one agent: many units, phases, standing coordinator. `playbooks/orchestrate.md`.
