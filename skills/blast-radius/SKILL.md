...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Pull the PR and commits for full context.
...
6. For a big or wide change, spawn one subagent per model in the `interrogate` skill's reviewer roster, put the same question to each, and merge the answers. Different models catch different real bugs.
