...
7. `fallback: claude`: read `exit code` before you spawn anything.
   ...
   - Anything else. Spawn `pstack:developer` or `pstack:reviewer` with the same brief. A writer fallback works in the named `worktree` when that line is not `(none)` and reads git state there first; a partial `report.md` at the run path is worth reading. When `exit code: timeout` sent you here, run `pgrep -af "codex exec"` and confirm no run still holds that worktree before the fallback writer touches it.
