...
**You own the run.** Claude Code only; skip this playbook on any other harness. The watcher's own steps are `references/codex-watcher-body.md`, which is the whole body of both watchers, and `hooks/codex_watcher_guard.py` locks it to exactly those commands. This page is the owner's half. The watcher runs `codex-agent`, a dedicated Codex install with its own `CODEX_HOME`, so a watcher run never shares the user's interactive Codex session, config, or authentication state.
...
7. `fallback: claude`: read `exit code` before you spawn anything.
   ...
   - Anything else. Spawn `pstack:developer` or `pstack:reviewer` with the same brief. A writer fallback works in the named `worktree` when that line is not `(none)` and reads git state there first; a partial `report.md` at the run path is worth reading. When `exit code: timeout` sent you here, run `pgrep -af "codex exec"` (`codex-agent` execs the codex binary, so a live run's process image shows as `codex`, never as `codex-agent`) and confirm no run still holds that worktree before the fallback writer touches it.
