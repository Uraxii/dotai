...
- Every writer gets its own git worktree on its own branch, at `.nikki-agents/worktrees/<name>` in the project, on branch `agent/<name>`. Where the worktree lives is the rule. How it gets there differs by harness. On Claude Code, spawn with `isolation: "worktree"` on the `Agent` tool and the native worktree feature places it there. On every other harness the agent runs `git worktree add` to that same path itself. The main checkout is read-only for agents; the rule holds by instruction, not by an enforcing hook. Only the coordinator lands a verified branch, fast-forward or cherry-pick.
