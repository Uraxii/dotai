...
- Every writer gets its own git worktree on its own branch. On Claude Code that means spawning with `isolation: "worktree"` on the `Agent` tool, never `git worktree add` by hand; the main checkout is read-only for agents. Only the coordinator lands a verified branch, fast-forward or cherry-pick.
