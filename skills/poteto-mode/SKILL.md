...
- Every writer gets its own git worktree on its own branch, at `.nikki-agents/worktrees/<name>` in the project, on branch `agent/<name>`. Where the worktree lives is the rule. How it gets there differs by harness. On Claude Code the `WorktreeCreate` hook places it when you spawn with `isolation: "worktree"` on the `Agent` tool. On every other harness the agent runs `git worktree add` to that same path itself. The main checkout is read-only for agents, and a write from a worktree at any other path is denied wherever the isolation hook runs, which is Claude Code and Copilot CLI. Codex wires no hooks, so there the rule holds by instruction alone. Only the coordinator lands a verified branch, fast-forward or cherry-pick.
