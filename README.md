...
| `hooks/` | Hook scripts. `cap_bash_timeout.py` is a `PreToolUse` gate on long Bash timeouts; it is registered nowhere and does not run. Nothing installs them, so copy one into `~/.claude/hooks/` and wire it yourself. `agent-guard/guard.py` denies a helper agent's write into the main checkout; the plugin install wires it automatically via `hooks/hooks.json` (Claude) and repo-root `hooks.json` (Copilot). |
