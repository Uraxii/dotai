...
**You own the run.** Claude Code only; skip this playbook on any other harness. The watcher carries its own steps in `plugins/pstack-nikki/agents/developer-codex.md` and `plugins/pstack-nikki/agents/reviewer-codex.md`, and `hooks/codex_watcher_guard.py` locks it to exactly those commands. This page is the owner's half.

1. Reach for Codex when the unit is one scoped implementation (`pstack-nikki:developer-codex`) or one review gate (`pstack-nikki:reviewer-codex`). Multi-kind work, tests, search, and orchestration stay on Claude: the watcher holds only Bash and Write, and the run starts with `-c agents.enabled=false`, so Codex does the brief itself and spawns no helpers. Codex already known unavailable this session: spawn plain `pstack-nikki:developer` or `pstack-nikki:reviewer` instead.
...
3. Open the watcher's prompt with this header, then the brief:
   ...
   `kind` is `writer` or `reviewer`. `worktree` is `create` or an existing absolute path, and a reviewer omits it. Take `model` from `plugins/pstack-nikki/models.json`: the first `codex` entry of the `feature, refactoring` row for a writer, of `judgment and prose` for a reviewer.
...
7. `fallback: claude`: spawn `pstack-nikki:developer` or `pstack-nikki:reviewer` with the same brief. A writer fallback works in the named `worktree` when that line is not `(none)` and reads git state there first; a partial `report.md` at the run path is worth reading. When `exit code: timeout` sent you here, run `pgrep -af "codex exec"` and confirm no run still holds that worktree before the fallback writer touches it.
