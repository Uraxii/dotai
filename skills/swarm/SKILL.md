...
5. Give each worker its own writable output when it writes. A worker that writes in the repo gets its own git worktree on its own branch, never a shared checkout; otherwise `/tmp/swarm-<slug>/worker-<n>/`.
