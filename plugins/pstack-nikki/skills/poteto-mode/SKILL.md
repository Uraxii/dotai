...
- **Delegate to Codex.** Claude Code only; skip it on any other harness. The watcher body of `developer-codex` and `reviewer-codex`: run a brief as a real Codex session on a GPT model and copy the results back verbatim; the owner handles fallback (Agents above). `playbooks/delegate-to-codex.md`.
