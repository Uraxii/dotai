...
| `hooks/` | Hook scripts. `cap_bash_timeout.py` is a `PreToolUse` gate on long Bash timeouts; it is registered nowhere and does not run. Nothing installs them, so copy one into `~/.claude/hooks/` and wire it yourself. `agent-isolation/agent_isolation.py` denies a helper agent's write into the main checkout, or from any worktree outside `.nikki-agents/worktrees/`. On Claude Code the `WorktreeCreate` and `WorktreeRemove` hooks put every agent worktree at `<project>/.nikki-agents/worktrees/<name>` on branch `agent/<name>` and delete it again on teardown. The plugin install wires all of it via `hooks/hooks.json` (Claude) and repo-root `hooks.json` (Copilot). Add `.nikki-agents/` to the exclude config of your editor, LSP, and any semantic index: `.git/info/exclude` covers git, ripgrep, and fd, but an indexer that keeps its own ignore list walks the worktrees and ends up crawling six figures of files in a repo with a few hundred tracked ones. |
