...
| `hooks/` | Hook scripts. `cap_bash_timeout.py` is a `PreToolUse` gate on long Bash timeouts; it is registered nowhere and does not run. `handoff-token-flag.py` warns before context compaction; plugin installation wires it for Claude Code, Codex, and Copilot CLI. `agent-isolation/agent_isolation.py` denies a helper agent's write into the main checkout, or from any worktree outside `.nikki-agents/worktrees/`. On Claude Code `agent-isolation/place_worktree.py` puts every agent worktree at `<project>/.nikki-agents/worktrees/<name>` on branch `agent/<name>` and deletes it again on teardown. Plugin installation wires isolation via `hooks/hooks.json` for Claude Code and the root `hooks.json` for Copilot CLI. Add `.nikki-agents/` to the exclude config of your editor, LSP, and any semantic index: `.git/info/exclude` covers git, ripgrep, and fd, but an indexer that keeps its own ignore list walks the worktrees and ends up crawling six figures of files in a repo with a few hundred tracked ones. |
