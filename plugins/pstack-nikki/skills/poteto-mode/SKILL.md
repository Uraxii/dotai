...
Every spawn carry the brief fields in `references/brief.md`. Field you cannot fill = task not scoped. Model pinned per call from `plugins/pstack-nikki/models.json`, never frontmatter. A harness alias naming the same model count as pinnable (`claude-opus-5` -> `opus`, `claude-sonnet-5` -> `sonnet` on harnesses that only take aliases). Constraints live in FORBIDDEN, not tool config. Paste the user's global instructions into every spawn; directives decay.
